...
class TransEncoder(nn.Module):
    def __init__(self, num_attn_layers, hidden_dim=128):
        ...
        attn_layer = attnLayer(hidden_dim)
        self.layers = _get_clones(attn_layer, num_attn_layers)
        self.position_embedding = nn.Parameter(torch.randn(1026, 1, 256))  #    torch.Size([1026, 2, 256])

    def forward(self, imgf):  # torch.Size([1026, 2, 256])
        pos = self.position_embedding.cuda()
        for layer in self.layers:
...
class PreTrain(nn.Module):
    def __init__(self):
        ...
        # para
        self.para_tail = nn.Sequential(
            nn.LayerNorm(self.dim),
            nn.Linear(self.dim, self.num_para)
        )
    ...
    def forward(self, image1):
        ...
        imfmap = torch.relu(imfmap)  # torch.Size([2, 256, 32, 32])
        B, c, h, w = imfmap.shape
        imfmap = imfmap.flatten(2).permute(2, 0, 1)   # torch.Size([1024, 2, 256])

        cls_token = self.cls_token.expand(B, -1, -1).permute(1, 0, 2)  # torch.Size([1, 2, 256])
        para_token = self.para_token.expand(B, -1, -1).permute(1, 0, 2)  # torch.Size([1, 2, 256])

        imfmap = torch.cat([imfmap, cls_token, para_token], dim = 0)  # torch.Size([1026, 2, 256])

        imfmap = self.bm_encoder(imfmap)  # torch.Size([1026, 2, 256])

        cls_token = imfmap[1024]   # torch.Size([2, 256])
        para_token = imfmap[1025]

        # cls_token
        cls = self.cls_tail(cls_token)  # torch.Size([2, 3])

        # para_token
        para = self.para_tail(para_token)
        return cls, para
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        outs = net(inputs1)
        print(outs[0].shape, outs[1].shape, outs[2].shape)  # torch.Size([2, 3, 288, 288])
